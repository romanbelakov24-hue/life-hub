 ...
 *
 * С 18.09.2026 — открыта всем: общий Telegram-бот пишет в аккаунт life hub, и
 * без своего аккаунта им не воспользоваться. Данные разделены по user_id —
 * новый пользователь не видит чужого. Закрыть регистрацию без правки кода —
 * переменная REGISTRATION_CLOSED=true (vars в wrangler.jsonc); первый аккаунт
 * в пустой базе создаётся в любом случае, иначе новую установку не с чего начать.
 */
export async function isRegistrationOpen(): Promise<boolean> {
  if (process.env.REGISTRATION_CLOSED !== "true") return true;
