...
/**
 * Показатели здоровья: последние значения + ритм шагов за две недели.
 *
    ...
    <Panel index={1}>
      <PanelHeader
        ...
        description={
          latest
            ? undefined
            : "Появятся после первого запуска автоматизации из инструкции выше"
        }
      />
      ...
      {latest ? (
        <>
          <div className="mt-4 grid grid-cols-3 gap-4">
            ...
            <Metric
              label="Сон"
              value={latest.sleepMinutes === null ? "—" : formatSleep(latest.sleepMinutes)}
          ...
          {recent.some((day) => day.steps !== null) ? (
            <div className="mt-6 border-t border-line pt-4">
              <p className="eyebrow mb-3">Шаги за 14 дней</p>
                    ...
                    style={{
                      height:
                        day.steps && day.steps > 0
                          ? `${(day.steps / maxSteps) * 100}%`
                          : "3px",
                      backgroundColor: day.steps ? undefined : "var(--line)",
                    }}
                  />
                ))}
              </div>
